...
3. We implemented a simple numerical IK solver. What are the advantages and disadvantages compared to an analytical IK solver? _An advantages is that it is one algorithm for any configuration, easy to incoperate extra tasks using nullspace of jaccobian, can add damping to handle singularities, while a disadvantages is that it requires many iterations compared to instantaneous analytic solution for example while also only providing one valid solution near the starting state_
4. What are the limits of our IK solver compared to state-of-the-art IK solvers? _to guarantee staying in joint limits, we need to use clamping (can result in error) which works for this example, state-of-the-art uses mathematical guarantees to stay within limits, also utilizing multiple restarts to achieve a much higher probability to find global minimum solution_
