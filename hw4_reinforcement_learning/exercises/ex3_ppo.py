...
class PPOAgent:
    ...
    def select_action(self, obs: torch.Tensor):
        ...
        with torch.inference_mode():
            ...
            value = self.critic.forward(obs).item()
    ...
    def compute_kl_mean(self, old_mu_batch, old_std_batch, mu_batch, std_batch):
        ...
        kl_per_dim = torch.sum(torch.log(std_batch / old_std_batch) + (torch.pow(old_std_batch, 2) + torch.pow(old_mu_batch - mu_batch, 2)) / (2 * torch.pow(std_batch,2)) - 0.5, dim=-1)
        kl_per_sample = torch.mean(kl_per_dim, -1)
    ...
    def compute_surrogate_loss(self, logp_batch, old_logp_batch, adv_batch):
        ...
        ratio = torch.exp(logp_batch - old_logp_batch) # numerically better than normal formula
        clipped_ratio = torch.clamp(ratio, 1 - self.clip_ratio, 1 + self.clip_ratio)
        objective = torch.min(ratio * adv_batch, clipped_ratio * adv_batch)
        surrogate_loss = -torch.mean(objective, dim=0) # we want to maximize it
    ...
    def compute_value_loss(self, val_batch, old_val_batch, ret_batch):
        ...
        value_loss_unclipped = torch.pow(val_batch - ret_batch, 2)
        value_clipped = old_val_batch + torch.clamp(val_batch - old_val_batch, -self.clip_ratio, self.clip_ratio)
        value_loss_clipped = torch.pow(value_clipped - ret_batch, 2)
    ...
    def compute_entropy_loss(self, entropy_batch):
        ...
        return -self.entropy_coeff * entropy_batch.mean() # negate since we want to maximize
